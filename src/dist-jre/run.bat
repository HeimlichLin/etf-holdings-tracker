@echo off
REM ETF Holdings Tracker - Windows Launcher with Embedded JRE
...
REM Use embedded JRE
set "JAVA=%APP_HOME%jre\bin\java.exe"

REM Check if embedded JRE exists
if not exist "%JAVA%" (
    echo Error: Embedded JRE not found at %JAVA%
...
echo Starting ETF Holdings Tracker...
"%JAVA%" ^
    -Xms128m ^
    -Xmx512m ^
    --enable-preview ^
